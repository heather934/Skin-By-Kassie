 ...
 *
 * Cloudflare Access sits in front of these routes and is the primary lock —
 * this middleware is the second one. It verifies the signed token Access
 * attaches to each request, so even if the Access policy were removed or
 * misconfigured, the API stays shut.
 *
 * Required environment variables (set in the Pages dashboard):
 *   ACCESS_TEAM_DOMAIN  e.g. skinbyklb.cloudflareaccess.com
 *   ACCESS_AUD          the Application Audience tag from the Access app
 *   ADMIN_EMAIL         Kassie's email — the only address allowed through
 */
...
async function verifyAccessToken(token, env) {
  ...
  // Audience must match this specific Access application
...
// Defense-in-depth against CSRF: the Access session cookie rides along on any
// cross-site request automatically, so a state-changing request (anything but
// GET) must also show it actually originated from this site. A forged request
// from another page won't carry a matching Origin header.
